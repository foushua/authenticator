...
class Authenticator
{
	...
	public function __construct(string $hostname, string $name, string $username, string $password, int $port = 3306)
	{
		try {
			...
			if (isset($_SESSION['user'])) $this->authenticated = true;
	...
	{
        if (!$this->validate($email, FILTER_VALIDATE_REGEXP, [ 'regexp' => '/[\s\S]/' ])) throw new Exception("Please fill an email address..");
        if (!$this->validate($password, FILTER_VALIDATE_REGEXP, [ 'regexp' => '/[\s\S]/' ])) throw new Exception("Please fill your password..");
        if (!$this->validate($email, FILTER_VALIDATE_EMAIL)) throw new Exception("The format of your email address does not seem correct..");
		if (!$this->validate($password, FILTER_VALIDATE_REGEXP, [ 'regexp' => '/^.{8,}$/' ])) throw new Exception("Your password must contain at least 8 characters");
		...
		try {
			...
			if (empty($user)) throw new Exception("No users found with this email..");
    ...
    {
        if (!$this->validate($email, FILTER_VALIDATE_REGEXP, [ 'regexp' => '/[\s\S]/' ])) throw new Exception("Please fill an email address..");
        if (!$this->validate($password, FILTER_VALIDATE_REGEXP, [ 'regexp' => '/[\s\S]/' ])) throw new Exception("Please fill a password..");
        if (!$this->validate($email, FILTER_VALIDATE_EMAIL)) throw new Exception("The format of your email address does not seem correct..");
        if (!$this->validate($password, FILTER_VALIDATE_REGEXP, [ 'regexp' => '/^.{8,}$/' ])) throw new Exception("Your password must contain at least 8 characters");
        if ($password != $confirm) throw new Exception("Passwords don't match..");
